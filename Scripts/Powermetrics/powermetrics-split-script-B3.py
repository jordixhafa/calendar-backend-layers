...
def run_setup_test():
    print("[INFO] Running setup test to create users/events...")
    try:
        subprocess.run(["npm", "test", "--", "B3_1_notification_setup.test.ts"], check=True)
        print("[INFO] Setup test completed successfully.")
    except subprocess.CalledProcessError:
        print("[ERROR] Setup test failed. Aborting run.")
        return False
    return True

def run_powermetrics_and_notification_test(run_index):
...
def main():
    ...
    for i in range(NUM_RUNS):
        ...
        log_path, elapsed_time = run_powermetrics_and_notification_test(i)
